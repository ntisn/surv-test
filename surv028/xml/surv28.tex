...
\addtocline{subsection}{A strategy to obtain the matrix fundamental solution $\psi$}
...
\section[Fundamental Matrices]{Fundamental matrices}\label{sec2}

\addtocline{subsection}{The set $\sum$ and the associated ordering in a sector}

\addtocline{subsection}{The fundamental matrices $m$ and $\tilde{m}$: definition and uniqueness}


The eigenvalue equation $Lu=z^{n}u$ can be reduced to a first order system
\renewcommand{\theequation}{\thesection.\arabic{equation}}
\setcounter{equation}{0}
...
\def\thesection{\arabic{section} bis}
\section{The Second Order Case}

\addtocline{subsection}{Jost functions and scattering data}

\addtocline{subsection}{Relation to the fundamental matrices of \S\ref{sec2}}



When $n=2,$ the set $\Sigma$ is $\mathbf{R}$. The equations (\hyperref[eq2.22-]{2.22}) give us two solutions for $z\notin \mathbf{R}$: \index{notation}{$u_{1}, \overset{\sim}{{u}_{2}}$}
...
The equations (\hyperref[eq2.22-]{2.22}) make sense for $z\in \mathbf{R}$ and give boundary values\index{notation}{$u_{1}^{\pm},\ u_{2}^{\pm}$}
...
\section[Fundamental Tensors]{Fundamental tensors}\label{sec3}

\addtocline{subsection}{The fundamental tensors $f_{k}(x,z) = m_1 \wedge m_2 \wedge \cdots \wedge m_k$ and $g_k(x,z) = m_k \wedge m_{k+1} \wedge \cdots \wedge m_n$ }

\addtocline{subsection}{Volterra equations, existence and uniqueness in an open sector $\Omega$; smoothness and extendability to $\overline{\Omega}\backslash 0$}
...
The $2n$ problems (\hyperref[eq3.17-]{3.17}) can be discussed by considering a single\index{subject}{abstract (Volterra) model, (3.21)} \emph{abstract model}. Let $V$ be a finite-dimensional hermitian vector space with norm $\Vert\,\Vert$ and let $A\in \mathscr{L}(V)$ be a normal operator. Let\index{notation}{$\Omega_{A}$}
...
\section[Behavior of Fundamental Tensors as $|x|\rightarrow\infty$; the Functions $\Delta_{k}$]{Behavior of fundamental tensors as $|x|\rightarrow\infty$; the functions $\Delta_{k}$}\label{sec4} 

\addtocline{subsection}{Asymptotics of $f_k$ as $x\rightarrow{+}\infty$ and of $g_k$ as $x\rightarrow{-}\infty$}

\addtocline{subsection}{The functions $\Delta_{k}$ existence, analyticity in $\Omega$, and extendability to $\overline{\Omega}$; integral representations}
...
\section[Behavior of Fundamental Tensors as $ z\rightarrow\infty$]{Behavior of fundamental tensors as $ z\rightarrow\infty$}

\addtocline{subsection}{Asymptotics of $f_k$ and $g_k$ as $z\rightarrow\infty$}

\addtocline{subsection}{Asymptotics of $\Delta_{k}$ as $z\rightarrow\infty$}
...
\section[Behavior of Fundamental Tensors as $z\rightarrow 0$]{Behavior of fundamental tensors as $z\rightarrow
0$}\label{sec6}


\addtocline{subsection}{Asymptotics of $f_k$ and $g_k$ as  $z\rightarrow0$}

\addtocline{subsection}{Asymptotics of $\Delta_{k}$ as $z\rightarrow0$}


There are two competing features of the problem
...
\end{proof}
\section[Construction of Fundamental Matrices]{Construction of fundamental matrices}\label{sec7}

\addtocline{subsection}{Decomposition of $f_k(g_k)$ into $m_j$'s ($\tilde{m}_j$'s)}

\addtocline{subsection}{The relation between the poles of $m_{k}$ and the zeros of $\Delta_{k-1}$}

\addtocline{subsection}{The residue of $m$ at a pole}

Starting from the fundamental tensor families $\{f_{k}, g_{k}\}$ for $L$ on a sector $\overline{\Omega}\backslash 0$, we want to construct the columns of the matrices $m$ and $\tilde{m}$, i.e., the solutions to (\ref{eq2.20})--(\hyperref[eq2.21-]{2.21}). The first step is to show that the fundamental \index{subject}{decomposable tensors, (7.1)}\index{subject}{tensors, decomposable, (7.1)}tensors are \emph{decomposable}, i.e.,
\begin{equation*}
...
\end{equation}
Then $L$ has an eigenfunction $0\neq u\in L^{2}(\mathbf{R})$ with eigenvalue $z^{n}$.\qed
\end{cor}
...
\end{equation*}
\end{remark}
\section[Global Properties of Fundamental Matrices; the Transition Matrix $\delta$]{Global properties of fundamental matrices; the transition matrix $\delta$}\label{sec8}

\addtocline{subsection}{Relation between $m$ and $\tilde{m}$: the matrix $\delta$}

\addtocline{subsection}{Asymptotics of $m$ as $z\rightarrow\infty$ and as $z\rightarrow0$}

\addtocline{subsection}{Asymptotics of $\frac{\partial^{j}}{\partial z^{j}}\psi(x, 0)$ as $x\rightarrow\pm\infty$}
...
\end{equation*}
Moreover,\index{notation}{$\delta(z)$}
\setcounter{equation}{1}
\begin{equation}\label{eq8.2}
m(x, z)=\tilde{m}(x, z)\delta(z),\quad \text{where } \delta(z)=\left[\begin{array}{cccc}
\Delta_{1}/\Delta_{0} &                            &           &\\
                               & \Delta_{2}/\Delta_{1} &           &\\
                               &                            & \ddots  &\\
                               &                            &           & \Delta_{n}/\Delta_{n-1}
...
\end{theorem}
\begin{proof}
Theorems~\ref{chap01:thm7.15} and~\ref{thm7.24} give the existence of $m$ and $\tilde{m}$ on the set (\ref{eq8.3}), and Corollary ~\ref{coro7.25} gives the relation (\ref{eq8.2}). Smoothness and holomorphy follow from the construction, together with Lemma ~\ref{chap01:lem7.11}. Boundedness of the set $Z$ is a consequence of Corollary \ref{chap01:coro5.18}: the $\Delta_{k}$ converge to 1 at $\infty$. Finally, given $z\in Z$ choose $k$ such that $\Delta_{k-1}(z)=0,\ \Delta_{k}(z)\neq 0$. Then Corollary \ref{chap01:coro5.18} implies that $m_{k}(x, \cdot)$ has a pole of positive order at $z.\quad$
\end{proof}
\setcounter{theorem}{3}
...
\section[Symmetries of Fundamental Matrices]{Symmetries of fundamental matrices}

\addtocline{subsection}{The $\alpha$-symmetry of $m$, $\tilde{m}$, and $\Delta_{k}$}

\addtocline{subsection}{The relation between $m_{L^{*}}(x,z)$ and $m_{L}(x,z)$}
...
\section[The Green's Function for $L$]{The Green's function for $L$}\label{sec10}

\addtocline{subsection}{The Green's kernel expressed in terms of the normalized eigenfunctions of $L$ and $L^{*}$}


We may use the fundamental matrix $m_{L}$ and $m_{L*}$ to construct the (bounded) Green's function for $L$: for $ z\in \mathbf{C}\backslash \Sigma$, we want to find $G(x, y, z)$ such that $G(\cdot,y, z)$ is a bounded solution of\index{notation}{$G(x, y, z)$}
\setcounter{equation}{0}
...
\section[Generic Operators and Scattering Data]{Generic operators and scattering data}\label{sec11}

\addtocline{subsection}{Definition of genericity}

\addtocline{subsection}{Definition of scattering data}

\addtocline{subsection}{Injectivity of the map between generic operators and scattering data}
...
In the rest of this section we assume that $L$ is generic. By (11.4) the $\Delta_{k}$ are nonzero near $z=0$ and by (\ref{eq11.3}) they are nonzero on the boundary of $\Omega$, away from $0$. Since they are holomorphic and close to 1 for large $z$ (Corollary \ref{chap01:coro5.18}), \emph{each} $\Delta_{k}$ \emph{has only finitely many zeros}. Set\index{notation}{$Z$}\index{notation}{$Z_{k}$}
\setcounter{equation}{5}
...
L\mapsto v
\end{equation*}
(for generic $L$) \emph{is the solution to the forward problem}. The \emph{inverse problem}\index{subject}{inverse problem, (11.17)} is the problem of recovering $L$ from $v;$ it is the subject of Part \ref{partII}. Here we show that the inverse problem is meaningful in principle.
\end{definition}
\setcounter{theorem}{17}
...
\section[Algebraic Properties of Scattering Data]{Algebraic properties of scattering data}\label{sec12}

\addtocline{subsection}{The $\alpha$-symmetry of $v$}

\addtocline{subsection}{The block structure of $v$ on $\sum$}

\addtocline{subsection}{Diagonal entries of $v$ in terms of $\gamma$}

\addtocline{subsection}{Integral representation of the entries of $v$}
...
\section[Analytic Properties of Scattering Data]{Analytic properties of scattering data}\label{sec13}

\addtocline{subsection}{Smoothness and decay properties of $v$ on $\sum$}

\addtocline{subsection}{Properties of the asymptotic expansion of $v$ at $z = 0$}
...
\end{equation}
is smooth. By Theorem ~\ref{thm8.15}, $m_{\pm} d_{z}^{-1}$ are smooth up to $z=0$. The determinants are independent of $z$ (on a fixed ray). Therefore
\begin{equation}\label{eq13.3}
d_{z}v(z)\pi_{j}d_{z}^{-1}=(m_{-}(0, z)d_{z}^{-1})^{-1}m_{+}(0,\ z)d_{z}^{-1}
\end{equation}
is smooth up to $z=0$. For the $2\times 2$ block $V$ in Theorem ~\ref{thm12.17} this means that
\begin{align*}
\left(\begin{array}{ll}
a & z^{-1}(1+ab)\\
z & \qquad b
...
\section[Scattering Data for $\tilde{m}$; Determination of $\tilde{v}$ from $v$]{Scattering data for $\tilde{m}$; determination of $\tilde{v}$ from $v$}\label{sec14}

\addtocline{subsection}{Definition of $\tilde{v}$ and its relation to $v$}

\addtocline{subsection}{Obtaining $\gamma$ from $v$ as the solution of a factorization problem}
...
\end{remark}
\section[Scattering Data for $L^{*}$]{Scattering data for $L^{*}$}\label{sec15}

\addtocline{subsection}{Relations between $v_{L}$ and $v_{L^{*}}$ and between $\Delta_{L}$ and $\Delta_{L^{*}}$}
...
\end{equation*}
The desired identity is an easy consequence of these identities together with (\ref{eq15.14}).
\end{proof}

\section[Generic Selfadjoint Operators and Scattering Data]{Generic selfadjoint operators and scattering data}\label{sec16}

\addtocline{subsection}{Definition of generic selfadjoint operator}

\addtocline{subsection}{$\Delta_{k}$ and $v$ arising from selfadjoint operators }

\addtocline{subsection}{Forbidden regions}
...
\end{theorem}
The proof of Theorem \ref{them16.3} is given in \S\S 18 and 19.
...
Suppose $L$  has odd order $n=2l+1$.  Then $Z_{l}$ $($resp. $Z_{l+1})$ does not intersect any of the closed sectors with angle $\exp(i\pi/n)$ which are bisected by the rays $\Sigma_{j}$, where $j$ and $l$ have the same parity $($resp. opposite parity$)$. $($See Figure~$\ref{fig5}.)$  Moreover, no $Z_{k}$  intersects the boundaries of these sectors.\index{subject}{forbidden regions, Fig. 5}
\end{theorem}
...
Suppose $z_{0}\in\Sigma_{j}\cap Z_{l}$, so $j$ and $l$ have opposite parity. As in the proof of Theorem \ref{thm15.3} there are matrices $v_{\pm}(z_{0})$ characterized by the conditions that for each $x$\index{notation}{$v_{\pm}(z_{0})$}
\setcounter{equation}{9}
...
\section[The Green's Function Revisited]{The Green's function revisited}\label{sec17}

\addtocline{subsection}{The Green's function in the selfadjoint case}

\addtocline{subsection}{Derivation of the analyticity properties and the jump relations across $\sigma(L)$ from the algebraic properties of $v$}
...
\end{equation*}
The argument used to prove Theorem \ref{thm17.5} shows that $F(\lambda)$ is holomorphic off $\mathbf{R}$. Moreover, if $n=2l+1$ then the jump on $\mathbf{R}$ is
\begin{equation*}
...
\addtocline{subsection}{Obtaining generic behavior of $\Delta_{k}$'s at $z = 0$ by perturbation}


Our goal in this section and the next is to sketch proofs of Theorems \ref{thm11.5} and~\ref{them16.3}: the generic (selfadjoint) operators $L$ are dense among all (selfadjoint) operators. The topology is that induced by the map
\begin{equation*}
...
\addtocline{subsection}{Ensuring by perturbation that $\Delta_{k}$ has only simple zeros in a sector $\Omega$ and no roots on $\partial\Omega$}

\addtocline{subsection}{Ensuring by perturbation in the selfadjoint case that only allowable zeros occur on $\partial\Omega$}
...
If there is another pair $(l,m)$ satisfying (\ref{eq19.30}) it must be $(k-1, k+2)$, arranged as in Figure~\ref{fig7} or its reflection about $\mathbf{R}$. Thus $\beta_{l}=-\overline{\beta}_{k},\ \beta_{m}=-\overline{\beta}_{k+1}$. The relations (\ref{eq19.28}), (\ref{eq19.29}) over the range (\ref{eq19.30}) imply
\begin{equation*}
...
\caption{}\label{fig8}
\end{figure}
Thus $\beta_{l}+\beta_{m}=\overline{\beta}_{k}+\overline{\beta}_{k+1}=-(\beta_{l}+\beta_{m})$ and we obtain $\beta_{k+1}=-\beta_{k},\beta_{k+2}= -\beta_{k-1}$. This configuration arises solely in the allowable case: $n=2k$ and $z_{0}\in\Sigma_{j},j$ of opposite parity from $k$ (Figure~\ref{fig8}). As we know, this corresponds to an isolated negative eigenvalue $z_{0}^{n}$ which cannot be perturbed away from $\mathbf{R}_{-}$ by a selfadjoint perturbation; thus the procedure for moving singularities breaks down exactly where it should.
\end{proof}

In view of Remark \hyperref[subsec18.2]{18.2}, Theorem ~\ref{thm19.24} completes our discussion of genericity.

\section[Summary of Properties of Scattering Data]{Summary of properties of scattering data}\label{sec20}

\addtocline{subsection}{Definition of generic scattering data, and some immediate consequences}


 \index{notation}{$G_{0}(n)$, $G_{e}(n)$, $G(n)$}In this section we summarize the properties of scattering data for a \emph{generic selfadjoint operator L}, of order $n$. As we show in Part \ref{partII}, these properties actually characterize such data. The nonselfadjoint case is somewhat different, in that only a dense open subset of the matrix-valued functions we have described arises as scattering data; nevertheless, it is sufficiently similar that we do not lose much by concentrating on the selfadjoint problem.
...
\section[Normalized Eigenfunctions for Odd Order Inverse Data]{Normalized eigenfunctions for odd order inverse data}\label{sec21}\index{notation}{$H^{N}(\Sigma_{k})$, $H^{N}(\Sigma)$}\index{notation}{$S$ (scattering data)}


\addtocline{subsection}{Strategy for solving the inverse problem}

\addtocline{subsection}{Definition of the normalized eigenfunctions $\mu$}

\addtocline{subsection}{Statement of basic inverse theorem}


 Let $S$ belong\index{notation}{$L_{x}$} to $G_{0}(n)$, $n=2l + 1$. Straightforward calculations show that if $x\in \mathbf{R}$ then\index{notation}{$S_{x}$}
\renewcommand{\theequation}{\thesection.\arabic{equation}}
\setcounter{equation}{0}
...
\section[The Vanishing Lemma]{The vanishing lemma}\label{sec22}

\addtocline{subsection}{Null vectors and their triviality}

\addtocline{subsection}{Uniqueness of normalized eigenfunctions}
...
\addtocline{subsection}{The extension of $\mu$ to $(\Sigma\backslash 0)\cup Z$}

\addtocline{subsection}{The integral equation for $\mu$}

\addtocline{subsection}{Uniqueness of the solution of the integral equation}
...
\section[Factorization of $v$ near $z=0$ and Property (20.6)]{Factorization of $v$ near $z=0$ and property (20.6)}\label{sec25} \index{notation}{$T,\ T_{j}$}The main goal of this section is to prove the following result.\index{notation}{$\widehat{a}_{j}(z)$}
...
\section[Reduction to a Fredholm Equation]{Reduction to a Fredholm equation}\label{sec26}\index{notation}{$(a^{\#})_{x}^{\pm}$}


\addtocline{subsection}{Construction of a parametrix for the integral equation for $\mu(x, \cdot)$ using the factorization in \S\ref{sec25}}

\addtocline{subsection}{The change of variables $\mu\mapsto\mu^{\#}$}


\setcounter{section}{26}
\renewcommand{\thetheorem}{\thesection.\arabic{theorem}}
By the results of \S 24, the inverse problem reduces to the solution of the system (\ref{eq24.31})--(\ref{eq24.35}). First, rewrite equation (\ref{eq24.31}) as
\begin{align*}
...
\end{proof}\index{notation}{$h^{\#} (z)$}

\renewcommand\thetheorem{26.\arabic{theorem}}
\setcounter{theorem}{75}
\begin{definition}\label{eq26.76}\index{subject}{null vector, (\ref{eq26.76})}
...
\addtocline{subsection}{Solution of the integral equation for $h^{\#}-\mu^{\#}-1$ for negative $x$}
...
\addtocline{subsection}{Continuous extension of $\mu(x,\cdot)$ to the boundary of a sector}

\addtocline{subsection}{Asymptotics of $\mu(x,\cdot)$ as $ z\rightarrow0$}

\addtocline{subsection}{Smoothness with respect to $x$}


The first two results below establish that the normalized eigenfunction $\mu(x, \cdot)$ in (\ref{eq27.15}) has generic behavior as $z\rightarrow 0$ (cf. Theorem ~\ref{thm8.15}). The remainder of the section is concerned with the decay properties of $h^{\#}(x, \cdot)=\mu^{\#}(x, \cdot)-\mathbf{1}$ as $ x\rightarrow-\infty$ (see Proposition ~\ref{prop28.13} below).
...
\begin{proof}[Proof of Proposition 28.13]\label{pop28.13} The existence of the derivatives $d^{j}h^{\#}(x, \cdot)/dx^{j}$ in $Y_{1}$ (and in $Y_{0}$) follows from the formula $h^{\#} =(I-C_{x}^{\#})^{-1}r_{x}^{\#}$ and Lemma \ref{lem28.17}. Furthermore, $d^{j}h^{\#}/dx^{j}$ is a sum of terms each of which is a product of powers of $(I -C_{x}^{\#})^{-1}$ and the derivatives $d^{m}C_{x}^{\#}/dx^{m}, 0\leq m\leq j$, and $d^{m'}r_{x}^{\#}/dx^{m'},\ 0\leq m'\leq j$. For $x\leq x_{0}$, however, $\Vert(I-C_{x}^{\#})^{-1}\Vert_{L(Y_{0},Y_{0})}\leq 2$ and $\Vert d^{m}C_{x}^{\#}/dx^{m}\Vert_{L(Y_{0},Y_{0})}$ is bounded, $0\leq m\leq j\leq K-1$, by Lemma \ref{lem28.17}. Thus, for $0\leq j\leq K-1$,
...
\addtocline{subsection}{Asymptotics of $\mu(x, z)$ and $\mu^{\#}(x, z)$ as $z\rightarrow\infty$}

\addtocline{subsection}{Decay of the coefficients in the expansions as $x\rightarrow-\infty$}

\addtocline{subsection}{Decay of the $x$-derivatives of $\mu$, and $\mu^{\#}$ as $x\rightarrow-\infty$}
...
\addtocline{subsection}{Existence of an operator $L$ for which $\mu$ solves the generalized eigenvalue problem}

 Suppose first that $x<0$. Then each choice of $N, K$, and (small) $\varepsilon >0$, gives rise to a normalized eigenfunction $\mu(x, \cdot)$ for $S_{x}$, and hence they must all be the same by the Vanishing Lemma. In particular, the $a_{j}(x)$'s which appear in Proposition \ref{eq29.1}, and hence in Corollary \ref{eq29.8}, must be independent of $N$, $K$, and $\varepsilon$. Thus $m$ and $N$ in (\ref{eq29.5}) can be chosen as large as we please, which means that $a_{j}(\cdot)\in C^{\infty}$ and $a_{j}(x)\rightarrow 0$ rapidly as $ x\rightarrow-\infty$.
...
\section[The Scalar Factorization Problem for $\delta$]{The scalar factorization problem for $\delta$}\label{sec31}

\addtocline{subsection}{Existence of $\delta$: explicit formula in terms of $v$}
...
\section[The Inverse Problem at $ x=+\infty$ and the bijectivity of the map $L\mapsto S(L)=(Z(L), v(L))$]{The inverse problem at $ x=+\infty$ and the bijectivity of the map $L\mapsto S(L)=(Z(L), v(L))$}\label{sec32}

\addtocline{subsection}{Existence of the normalized eigenfunctions $\tilde{\mu}$ at $ x=+\infty$}

\addtocline{subsection}{Relation of $\tilde{\mu}$ to $\mu$}

\addtocline{subsection}{Selfadjointness of $L$}

\addtocline{subsection}{Bijectivity of the scattering map (odd order case)}


 Let $\eta(z)$ be as in Proposition ~\ref{prop31.3} and set
...
Define $\tilde{v}$ on $(\Sigma\backslash 0)\cup Z$ as follows (cf. Theorems \ref{thm14.4} and \ref{thm14.6}). If $z\in\Sigma\backslash 0$,\index{notation}{$\overset{\sim}{v}$}
...
\section[The Even Order Case]{The even order case}\label{sec33}

\addtocline{subsection}{Modification of the inverse procedure to include negative spectrum}
...
The construction of $\delta$ from $\eta$ now proceeds as in the odd dimensional case. Apart from repetitive details, this completes the proof of Theorem ~\ref{thm33.1}. \qed
...
\section[The Second Order Problem]{The second order problem}\label{sec34}

\addtocline{subsection}{Reduction of the inverse procedure to classical Faddeev-Marchenko theory; nongeneric data}
...
\addtocline{subsection}{The Cauchy problem for the Gelfand-Dikii evolutions}

\addtocline{subsection}{Well-posed and ill-posed problems; the geometry of phase space}


In this section, we use the results and methods of Parts \ref{partI} and \ref{partII} to analyze the Cauchy problem for the class of nonlinear wave equations\index{subject}{Gelfand-Dikii flows, (35.1)}\index{notation}{$H_{N,k}$}
\begin{equation*}
...
\addtocline{subsection}{Spectral decomposition of $L$ and the quantum mechanical wave operators in terms of bounded eigenfunctions}

\addtocline{subsection}{The scattering operator in terms of $v(L)$}
...
\end{proof}\index{notation}{$W_{\pm}$}
...
\section[Inserting and Removing Poles]{Inserting and removing poles}\label{sec37}

\addtocline{subsection}{Algebraic procedure to insert and remove poles of $\Delta_{k}$ while leaving the rest of the scattering data unchanged}

\addtocline{subsection}{B\"{a}cklund transformations for the Gelfand-Dikii evolutions}
...
\section[Matrix Factorization and First Order Systems]{Matrix factorization and first order systems}\label{sec38}

\addtocline{subsection}{Scattering data for ordinary differential operators viewed as scattering data for first order systems}

\addtocline{subsection}{Algebraic nature of the correspondence between operators and their associated first order systems}

\addtocline{subsection}{Nonuniqueness of the inverse problem for operators with coefficients in $L^{1}(\mathbf{R})$}

\addtocline{subsection}{The matrix factorization problem}

\addtocline{subsection}{Flows on systems induced by the Gelfand-Dikii hierarchy: modified KdV and others}

  The scattering data $S$ for an $n$th order ordinary differential operator $L=D^{n}+\sum_{j=0}^{n-2}p_{j}(x)D^{j}$ can also be viewed as (a special case of) scattering data arising from a first order system of the form
...
\end{figure}
In this section we consider various aspects of the relation between $L$ and (\ref{eq38.1}). Our main result (Theorem \ref{theo38.27} below) asserts that, as in \S 37, the map
\begin{equation*}
...
\bibitem[Ba2]{Ba2} \_\_\_\_\_\_\_, \emph{On the connection between phase shifts and scattering potential}, Rev. Mod. Phys. \textbf{21} (1949), 489--493.
...
\bibitem[BC2]{BC2} \_\_\_\_\_\_\_, \emph{Inverse scattering and evolution equations}, Comm. Pure Appl. Math. \textbf{38} (1985), 29--42.

\bibitem[BC3]{BC3} \_\_\_\_\_\_\_, \emph{Scattering and inverse scattering for first order systems}, II, Inverse Problems \textbf{3} (1987), 577--593.
...
\bibitem[Fa2]{Fa2} \_\_\_\_\_\_\_, \emph{Properties of the $S$-matrix of the one-dimensional Schr\"{o}dinger equation}, Trudy Mat. Inst. Steklov \textbf{73} (1964), 314--333; Amer. Math. Soc. Transl., vol. 65, Amer. Math. Soc., Providence, R. I., 1967, pp. 139--166.
...
\bibitem[H\"o]{Ho} L. H\"{o}rmander, \emph{An introduction to complex analysis in several variables}, Van Nostrand, New York, 1966.
...
\bibitem[SZ2]{SZ2} \_\_\_\_\_\_\_, \emph{Gauge theory of B\"{a}cklund transformations} II, Phys. D \textbf{26} (1987), 225--250.
